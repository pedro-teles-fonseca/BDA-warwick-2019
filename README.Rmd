...
We provide a self-contained Rstudio workflow that allows for the replicability of all the results in the poster. In order to direct R to the correct working directory you should open `bda-poster-warwick.Rproj` first. After that, all R scripts stored in the `R` folder should run smoothly.
...
* Plots comparing observed versus expected digit frequencies can be found in the `plots` folder. The code that generates the plots can be found in the `plots.R` script.

* The `aux-tables.pdf` file in the `tables-aux` folder is a user-friendly compilation of all the tables in the poster. The code that generates the results in those tables is in `aux-tables-bin.beta.R` and `aux-tables-mu-dir.R`.
